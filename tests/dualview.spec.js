...
// Vérifie que la version est 0.8.0 et que les scripts essentiels sont présents.

test('02 — package.json : version 0.8.0 et scripts corrects', () => {
    const raw = fs.readFileSync(path.join(ROOT, 'package.json'), 'utf8');
    const pkg = JSON.parse(raw);

    assert.equal(pkg.version, '0.8.0', 'Version attendue : 0.8.0');
