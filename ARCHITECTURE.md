...
| 1.0.0 | **Raccourcis souris étendus** (`main.js`, `landscape-tabs.js`, `landscape-settings.js`) : clic molette/Ctrl+clic ouvrent un lien en arrière-plan (`setWindowOpenHandler` lite `disposition`, `addTabWithUrl({background})`). **Boutons latéraux retour/avance réécrits** (`landscape-webview.js` `MOUSE_NAV_INJECT`, `landscape-pollers.js` `pollMouseNav()`) : abandon définitif des API Electron (`before-input-event`, `app-command`) confirmées non fonctionnelles dans l'architecture `<webview>` imbriquée (diagnostic terminal) → remplacement par injection DOM capturant `mousedown` directement dans la page + polling réutilisant `navBack()`/`navForward()` existants (cohérent avec sync portrait et pile de navigation v0.9.0, ~100 ms de latence acceptable). **Zoom Ctrl+molette fiabilisé** dans les webviews (`landscape-webview.js` `ZOOM_WHEEL_INJECT`, `landscape-pollers.js` `pollZoomWheel()`) : injection DOM capturant `wheel` avec `ctrlKey` + polling appelant `adjustZoom()` (même fonction que raccourcis clavier Ctrl+/−/0). **Fix favoris** (`favorites-manager.js`, `landscape-settings.js`, `landscape-i18n.js`) : ajout silencieusement refusé (domaines de connexion) affichait faussement un succès → vérification du retour booléen `favoritesAdd()` + toast d'avertissement `favoriteBlocked` ; `AUTH_DOMAINS_FAV` restreinte aux hôtes purement d'authentification → restauration de la capacité à mettre en favori profils/chaînes sur Twitter/X, Discord, Instagram, TikTok, Facebook, Steam. |
